...
class Word2VecModel:
    ...
    def __init__(self, dataset, vocab_size, embed_size, batch_size, num_sampled, learning_rate, mode='skip_gram'):
    ...
    def _import_data(self):
        ...
        with tf.name_scope('data'):
            ...
            self.input_words, self.label_words = self.iterator.get_next()
    ...
    def _create_embedding(self):
        ...
        with tf.name_scope('embed'):
            ...
            if self.mode == 'skip_gram':
                self.embed = tf.nn.embedding_lookup(self.embed_matrix, self.input_words, name='embedding')
                print(self.embed.get_shape())
            elif self.mode == 'cbow':
                self.embed = tf.reduce_mean(
                    tf.nn.embedding_lookup(self.embed_matrix, self.input_words, name='embedding'),
                    axis=1, name='embedding_sum')
                print(self.embed.get_shape())
            else:
...
def gen():
    yield from word2vec_utils.batch_gen(DOWNLOAD_URL, EXPECTED_BYTES, VOCAB_SIZE,
                                        BATCH_SIZE, SKIP_WINDOW, VISUAL_FLD, MODE)
...
def main():
    ...
    if MODE == 'skip_gram':
        ...
        dataset = tf.data.Dataset.from_generator(gen,
                                                 (tf.int32, tf.int32),
                                                 (tf.TensorShape([BATCH_SIZE, SKIP_WINDOW]),
                                                  tf.TensorShape([BATCH_SIZE, 1])))
    else:
        print('mode has to be either skip_gram or cbow')
    model = Word2VecModel(dataset, VOCAB_SIZE, EMBED_SIZE, BATCH_SIZE, NUM_SAMPLED, LEARNING_RATE, MODE)
    model.build_graph()
